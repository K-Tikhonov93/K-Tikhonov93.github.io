...
var level = Number(counter3)+Number(1);

let div = document.createElement('div');
div.className = "rules";
div.innerHTML = "<h3>Попытка №"+ level + "</h3><ul><li>На каждой чаше должно быть не менее двух гирь</li><li>На каждой чаше может быть не более четырех гирь</li><li>Числа от 1 до 9</li></ul>";
...
var armAngle = 0;
var leftWeight = [-2, -2, -2, -2];
var rightWeight = [-2, -2, -2, -2];
...
function drawArm() {
    ...
    if (armAngle >= 0) {
        ...
        for (var i = 0; i < 4; i++) {
            if (leftWeight[i] > -1) {
                ctx.drawImage(numbers[leftWeight[i]], -(arm.width / 2) - left_scale.width / 2 + armAngle - 30 + 50 * i, -(6 * arm.height) - armAngle * 4.5 + 200)
            }
        }
        ctx.drawImage(right_scale, (arm.width / 2) - left_scale.width / 2 - armAngle, -(6 * arm.height) + armAngle * 4 + 80);
        for (var i = 0; i < 4; i++) {
            if (rightWeight[i] > -1) {
                ctx.drawImage(numbers[rightWeight[i]], (arm.width / 2) - left_scale.width / 2 - armAngle - 30 + 50 * i, -(6 * arm.height) + armAngle * 4 + 200)
            }
        }
    }
    if (armAngle < 0) {
        ...
        for (var i = 0; i < 4; i++) {
            if (leftWeight[i] > -1) {
                ctx.drawImage(numbers[leftWeight[i]], -(arm.width / 2) - left_scale.width / 2 - armAngle - 30 + 50 * i, -(6 * arm.height) - armAngle * 4 + 200)
            }
        }
        ctx.drawImage(right_scale, (arm.width / 2) - left_scale.width / 2 + armAngle, -(6 * arm.height) + armAngle * 4.5 + 80);
        for (var i = 0; i < 4; i++) {
            if (rightWeight[i] > -1) {
                ctx.drawImage(numbers[rightWeight[i]], (arm.width / 2) - left_scale.width / 2 + armAngle - 30 + 50 * i, -(6 * arm.height) + armAngle * 4.5 + 200)
            }
        }
    }
    ctx.restore();
...
let timerId = setInterval(function() {
    ...
    if (now - startTime >= maxTime) {
        ...
        if (arraySum(leftWeight) == arraySum(rightWeight) && arraySum(leftWeight) > 0 && arrayCount(leftWeight)>1 && arrayCount(rightWeight)>1) {
            ...
            if (counter3==0){
                localStorage.setItem('lvl3', 900);
            }
            if (counter3==1){
                localStorage.setItem('lvl3', 600);
            }
            if (counter3==2){
                localStorage.setItem('lvl3', 300);
            }
            let div = document.createElement('div');
            div.className = "center";
            rect = [];
            div.innerHTML = "<h1>Игра окончена!</h1>";
            document.body.append(div);
            setTimeout(function() {
                        document.location.href = 'results.html'
                    }, (2 * 1000));
