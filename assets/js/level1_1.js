...
var level = Number(counter1)+Number(1);

let div = document.createElement('div');
div.className = "rules";
div.innerHTML = "<h3>Попытка №"+ level + "</h3><ul><li>На каждой чаше должно быть не менее двух гирь</li><li>На каждой чаше может быть не более трех гирь</li><li>Числа от 1 до 5</li></ul>";
...
let timerId = setInterval(function() {
    ...
    if (now - startTime >= maxTime) {
        ...
        if (arraySum(leftWeight) == arraySum(rightWeight) && arraySum(leftWeight) > 0 && arrayCount(leftWeight)>1 && arrayCount(rightWeight)>1) {
            clearInterval(timerId);
            //alert('Первый уровень пройден!');
            if (counter1==0){
                localStorage.setItem('lvl1', 300);
            }
            if (counter1==1){
                localStorage.setItem('lvl1', 200);
            }
            if (counter1==2){
                localStorage.setItem('lvl1', 100);
            }
            let div = document.createElement('div');
            div.className = "center";
            rect = [];
