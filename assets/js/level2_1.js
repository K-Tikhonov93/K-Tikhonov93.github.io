...
div.className = "rules";
div.innerHTML = "<h3>Попытка №"+ level + "</h3><ul><li>На левой чаше должно быть не менее двух гирь</li><li>На левой чаше может быть не более трех гирь</li><li>Числа от 1 до 7</li></ul>";
...
var rightWeight = randomInteger(2, 7);
...
function drawArm() {
    ...
    if (arraySum(leftWeight) == 0 && rightWeight != 0) {
        armAngle = 40;
    }
    if (rightWeight == 0 && arraySum(leftWeight) != 0) {
        armAngle = -40;
    }
...
let timerId = setInterval(function() {
    ...
    if (now - startTime >= maxTime) {
        ...
        if (arraySum(leftWeight) == rightWeight && arraySum(leftWeight) > 0 && arrayCount(leftWeight)>1) {
            clearInterval(timerId);
...
window.onmouseup = function(e) {
    if (mouse.x > 2 * canvas.width / 3 - arm.width / 2 - left_scale.width / 2 && mouse.x < 2 * canvas.width / 3 - arm.width / 2 - left_scale.width / 2 + left_scale.width) {
        if (leftWeight.indexOf(-2) != -1) {
            ...
            armAngle = 40 * (rightWeight - arraySum(leftWeight)) / maxWeight(rightWeight, arraySum(leftWeight));
        }
        counter++;
    };
